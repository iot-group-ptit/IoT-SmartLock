...
class MQTTService {
  ...
  // ========================================
  // SỬA LỖI TRONG mqtt.js
  // ========================================

  // 1. SỬA HÀM handleDeviceLogin() - Đảm bảo lưu session ĐÚNG CÁCH
  //   async handleDeviceLogin(data) {
  //     console.log("\n=================================");
  //     console.log("🔐 XỬ LÝ DEVICE LOGIN REQUEST");
  //     console.log("=================================");
  ...
  //       // ✅ SỬA: Lưu session VÀO Map với đầy đủ thông tin
  //       const sessionData = {
  //         session_token: sessionToken,
  //         device_id: device_id,
  //         logged_in_at: now,
  //         last_heartbeat: now,
  //         last_device_timestamp: Number(timestamp),
  //         status: "online",
  //       };

  //       this.deviceSessions.set(device_id, sessionData);

  //       // ✅ QUAN TRỌNG: Log để debug
  //       console.log("📝 Session đã lưu:");
  //       console.log("   Device ID:", device_id);
  //       console.log("   Session token:", sessionToken.substring(0, 16) + "...");
  //       console.log("   Total sessions:", this.deviceSessions.size);
  ...
  // ===== SỬA HÀM handleDeviceLogin() =====
  // Thay thế toàn bộ hàm handleDeviceLogin() trong mqtt.js
  ...
  async handleDeviceLogin(data) {
    ...
    try {
      ...
      // ✅ THÊM: Kiểm tra device đã login chưa
      const existingSession = this.deviceSessions.get(device_id);
      if (existingSession) {
        console.log("⚠️ Device đã có session:");
        console.log(
          "   Existing token:",
          existingSession.session_token?.substring(0, 16) + "..."
        );
        console.log("   Last heartbeat:", existingSession.last_heartbeat);
        ...
        if (timeSinceHeartbeat < this.SESSION_TIMEOUT) {
          console.log("   ✓ Session còn hiệu lực - Trả về session cũ");

          // Trả về session cũ (không tạo mới)
          return this.publish(this.topics.DEVICE_LOGIN_RESPONSE, {
            device_id,
            success: true,
            session_token: existingSession.session_token,
            message: "Already logged in - existing session reused",
            timestamp: new Date().toISOString(),
            auth_method: "x509_signature",
          });
        } else {
          console.log("   ✗ Session đã hết hạn - Xóa và tạo mới");
          this.deviceSessions.delete(device_id);
        }
      }
  ...
  // 2. SỬA HÀM handleDeviceHeartbeat() - Kiểm tra session ĐÚNG
  //   async handleDeviceHeartbeat(data) {
  //     const { device_id, session_token, status } = data;

  //     try {
  //       // ✅ THÊM: Log để debug
  //       console.log(`\n💓 Heartbeat từ ${device_id}`);
  //       console.log("   Session token:", session_token?.substring(0, 16) + "...");
  //       console.log("   Total sessions hiện tại:", this.deviceSessions.size);
  //       console.log(
  //         "   Danh sách device IDs:",
  //         Array.from(this.deviceSessions.keys())
  //       );
  ...
  //       if (!session) {
  //         console.log(`⚠️ Device ${device_id} không có session trong Map`);
  //         console.log("   → Có thể đã bị cleanup hoặc chưa login");
  ...
  //       // ✅ THÊM: Log session tìm được
  //       console.log("   ✓ Session tìm thấy:");
  //       console.log(
  //         "      Saved token:",
  ...
  //       // ✅ Verify session token
  //       if (session.session_token !== session_token) {
  //         console.log(`✗ Session token không khớp cho ${device_id}`);
  //         console.log(
  //           "   Expected:",
  //           session.session_token?.substring(0, 16) + "..."
  //         );
  //         console.log("   Received:", session_token?.substring(0, 16) + "...");

  //         // Xóa session cũ
  //         // this.deviceSessions.delete(device_id);

  //         // Đếm số lần token sai
  //         if (!session.invalid_token_count) {
  //           session.invalid_token_count = 0;
  //         }
  //         session.invalid_token_count++;

  //         console.log(
  //           `   ⚠️ Invalid token count: ${session.invalid_token_count}`
  //         );

  //         // Chỉ xóa session sau 3 lần token sai liên tiếp
  //         if (session.invalid_token_count >= 3) {
  //           console.log(`   ✗ Quá nhiều lần token sai - Xóa session`);
  //           this.deviceSessions.delete(device_id);
  //         }
  ...
  //       // ✅ Token đúng - Reset counter và cập nhật heartbeat
  //       session.invalid_token_count = 0;
  //       // ✅ Cập nhật heartbeat
  //       session.last_heartbeat = new Date();
  //       session.status = status || "online";
  ...
  //       console.log(`✓ Heartbeat hợp lệ - Status: ${status || "online"}`);
  ...
  // ===== SỬA HÀM handleDeviceHeartbeat() =====
  // Thay thế toàn bộ hàm handleDeviceHeartbeat()
  ...
  // 3. SỬA HÀM cleanupExpiredSessions() - THÊM LOG CHI TIẾT
  async cleanupExpiredSessions() {
    console.log("\n🧹 Cleaning up expired sessions...");
    console.log("   Total sessions trước cleanup:", this.deviceSessions.size);
    ...
    for (const [device_id, session] of this.deviceSessions.entries()) {
      ...
      console.log(`   Checking ${device_id}:`);
      console.log(`      Last heartbeat: ${session.last_heartbeat}`);
      console.log(
        `      Time since: ${Math.round(timeSinceHeartbeat / 1000)}s`
      );
      console.log(`      Timeout: ${this.SESSION_TIMEOUT / 1000}s`);

      // Nếu không heartbeat trong SESSION_TIMEOUT → xóa session
      if (timeSinceHeartbeat > this.SESSION_TIMEOUT) {
        this.deviceSessions.delete(device_id);
        cleanedCount++;
    ...
    console.log(`   Total sessions sau cleanup: ${this.deviceSessions.size}`);
    if (cleanedCount > 0) {
      console.log(`   ✓ Cleaned ${cleanedCount} expired session(s)`);
    } else {
  ...
  //   // 4. SỬA HÀM verifyDeviceSession() - Trả về object đầy đủ
  //   verifyDeviceSession(device_id, session_token) {
  //     console.log(`\n🔍 Verifying session for ${device_id}...`);
  //     console.log("   Token:", session_token?.substring(0, 16) + "...");
  ...
  //       Date.now() - new Date(session.last_heartbeat).getTime();
  //     console.log(
  //       `   Time since heartbeat: ${Math.round(timeSinceHeartbeat / 1000)}s`
  //     );
  ...
  // ===== SỬA HÀM verifyDeviceSession() - THÊM LOG =====
  // Thay thế hàm verifyDeviceSession()
  ...
  // ===== THÊM HÀM clearDeviceSession() =====
  // Thêm hàm mới này vào class MQTTService

  clearDeviceSession(device_id) {
    if (this.deviceSessions.has(device_id)) {
      this.deviceSessions.delete(device_id);
      console.log(`✓ Đã xóa session cho device ${device_id}`);

      // Gửi lệnh disconnect xuống ESP32
      this.publish(`smartlock/device/${device_id}/disconnect`, {
        device_id,
        reason: "Session cleared by server",
  ...
  //     const { cardUid, device_id, session_token } = data;

  //     try {
  //       if (device_id) {
  //         const verification = this.verifyDeviceSession(device_id, session_token);
  //         if (!verification.valid) {
  //           console.log(
  //             `✗ Device ${device_id} chưa xác thực: ${verification.reason}`
  //           );
